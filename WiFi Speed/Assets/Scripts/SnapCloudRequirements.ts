...
export class SnapCloudRequirements extends BaseScriptComponent {
  ...
  public showSetupInstructions: boolean = false
  ...
  private showWarning(): void {
    ...
    if (!this.supabaseProject) {
      this.log("Missing: Supabase Project — create via Window > Supabase > Import Credentials")
    }
  ...
  /**
   * Public HTTPS URL for a file in a public storage bucket.
   */
  public getPublicStorageUrl(bucket: string, objectPath: string): string {
    const base = this.getStorageApiUrl()
    if (base.length === 0) {
      return ""
    }
